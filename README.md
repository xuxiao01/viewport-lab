...
用户只需要打开内网部署的 Viewport Lab，不需要在本机克隆或启动 Viewport Lab。提交截图或 Agent 任务时，如果目标 URL 使用 `localhost`、`127.0.0.1` 或 `[::1]`，服务端会用当前 Web 请求的客户端内网 IP 替换回环地址。例如用户填写：

```text
http://localhost:5188
```

如果当前用户的内网地址是 `10.2.8.23`，部署机上的 Chromium 实际访问：

```text
http://10.2.8.23:5188
```

目标项目必须监听局域网地址，而不是只监听 `127.0.0.1`。以 Vite 项目为例：

```bash
pnpm dev -- --host 0.0.0.0
```

同时需要允许系统防火墙接受来自部署机的连接。如果请求经过额外的公司网关或反向代理，该代理也必须保留 `X-Forwarded-For` 客户端地址。
