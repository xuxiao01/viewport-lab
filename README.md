...
- Agent 探索仅在首台设备运行模型决策，并将每步工具调用同步广播到其他设备执行。
- Agent 每步动作后按 URL、加载、普通网络请求、DOM、字体、可见图片和连续渲染帧等待页面可观察，单步等待上限为 15 秒。
- Agent 自动保存动作后的完整 accessibility snapshot 和截图；模型侧快照按 SHA-256 去重，单次最多返回 40,000 字符。
...
Agent 批次以请求中的第一台设备作为主设备。模型只读取主设备的工具结果；其他设备原样执行相同命令和参数。从设备单步失败时会记录失败步骤和 warning 日志，并继续执行后续广播动作。

每台设备会独立执行页面就绪等待、快照和截图。等待超时不会把成功动作改为失败，系统仍会返回超时信号并保存当时的页面状态。完整快照始终写入本地 `snapshots/`；如果与该设备上一份快照完全一致，manifest 和模型上下文只记录复用引用，不重复保存树文本。超过 40,000 字符的模型侧快照会显式标记截断，Agent 可继续使用 `find` 检索页面内容。
